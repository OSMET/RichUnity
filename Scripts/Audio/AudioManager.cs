...
//            }
        ...
        public float GetAudioClassVolume(string audioClassName)
        {
            return GetAudioClassByName(audioClassName).Volume;
        }
        ...
        public void SetAudioClassVolume(string audioClassName, float volume)
        {
            ...
            if (audioClass != null)
            {
                ...
                if (!Mathf.Approximately(audioClass.Volume, volume))
                {
                    //value changed
                    audioClass.Volume = volume;
                    //Debug.Log(string.Format("[{0}] AudioClass: Volume Value changed to {1:0.00}", audioClass.Name,
                    //    volume));
                    ApplyAudioClassPropertiesToSources(audioClass);
                }
            }
        ...
        public bool GetAudioClassMuted(string audioClassName)
        {
            return GetAudioClassByName(audioClassName).Muted;
        }
